...
const fs = require('fs');
const Controller = require('../controller/CvSubmissionController');
const path = require('../app');
const fileConfigs = require('../configs/fileConfig');

let storageMaterial = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'public/documents/user/cv')
    },
...
Router.post('/upload-file', function (req, res) {
    uploadMaterial(req, res, function (err) {
        ...
        if (req.file) {
            res.json({
                file_url: `documents/user/cv/${req.file.filename}`
            });
        } else
